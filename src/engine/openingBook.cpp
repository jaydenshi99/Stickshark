...
// Standard Polyglot Random64 table (fixed across all Polyglot implementations).
...
// Engine piece index → Polyglot piece index
// Engine: WPAWN=0,WBISHOP=1,WKNIGHT=2,WROOK=3,WQUEEN=4,WKING=5,BPAWN=6,BBISHOP=7,BKNIGHT=8,BROOK=9,BQUEEN=10,BKING=11
// Polyglot: bpawn=0,wpawn=1,bknight=2,wknight=3,bbishop=4,wbishop=5,brook=6,wrook=7,bqueen=8,wqueen=9,bking=10,wking=11
static const int ENGINE_TO_POLY_PIECE[12] = {1, 5, 3, 7, 9, 11, 0, 4, 2, 6, 8, 10};

static inline int engineToPolySq(int sq) {
    // Engine: rank*8 + (7-file), Polyglot: rank*8 + file  (file 0=a)
...
uint64_t OpeningBook::computePolyglotHash(const Board& board) {
    ...
    for (int sq = 0; sq < 64; sq++) {
        ...
        hash ^= POLY_RAND[ENGINE_TO_POLY_PIECE[piece] * 64 + engineToPolySq(sq)];
    }

    uint8_t castling = board.history.top().castlingRights;
    if (castling & 1) hash ^= POLY_RAND[768]; // white kingside
    if (castling & 2) hash ^= POLY_RAND[769]; // white queenside
    if (castling & 4) hash ^= POLY_RAND[770]; // black kingside
    if (castling & 8) hash ^= POLY_RAND[771]; // black queenside

    int epCol = board.history.top().enpassantColumn;
    if (epCol != -1) {
        int epFile = 7 - epCol; // engine col→polyglot file (0=a)
        bool epPossible = false;
        if (board.turn) {
            // White to move: black pawn just pushed to rank 4; white pawn at rank 4 adjacent
            if (epCol > 0 && (board.pieceBitboards[WPAWN] & (1ULL << (4 * 8 + epCol - 1)))) epPossible = true;
            if (epCol < 7 && (board.pieceBitboards[WPAWN] & (1ULL << (4 * 8 + epCol + 1)))) epPossible = true;
        } else {
            // Black to move: white pawn just pushed to rank 3; black pawn at rank 3 adjacent
            if (epCol > 0 && (board.pieceBitboards[BPAWN] & (1ULL << (3 * 8 + epCol - 1)))) epPossible = true;
            if (epCol < 7 && (board.pieceBitboards[BPAWN] & (1ULL << (3 * 8 + epCol + 1)))) epPossible = true;
        }
        if (epPossible) hash ^= POLY_RAND[772 + epFile];
    }

    // XOR turn key when white to move
    if (board.turn) hash ^= POLY_RAND[780];

    return hash;
}

// Decode Polyglot move bits into engine (src, dst, promoFlag).
// Castling in Polyglot is king-to-rook; translate to king's actual destination.
std::tuple<int, int, int> OpeningBook::decodeMove(uint16_t polyMove) {
    int toFile   = (polyMove >> 0) & 7; // 0=a
    int toRank   = (polyMove >> 3) & 7; // 0=rank1
    int fromFile = (polyMove >> 6) & 7;
    int fromRank = (polyMove >> 9) & 7;
    int promo    = (polyMove >> 12) & 7; // 0=none,1=N,2=B,3=R,4=Q

    int engFrom = fromRank * 8 + (7 - fromFile);
    int engTo   = toRank   * 8 + (7 - toFile);

    // Detect castling (Polyglot: king moves to rook's original square)
    // White kingside: e1(3)→h1(0), translate to e1(3)→g1(1)
    if (engFrom == 3 && engTo == 0) return {3, 1, CASTLING};
    // White queenside: e1(3)→a1(7), translate to e1(3)→c1(5)
    if (engFrom == 3 && engTo == 7) return {3, 5, CASTLING};
    // Black kingside: e8(59)→h8(56), translate to e8(59)→g8(57)
    if (engFrom == 59 && engTo == 56) return {59, 57, CASTLING};
    // Black queenside: e8(59)→a8(63), translate to e8(59)→c8(61)
    if (engFrom == 59 && engTo == 63) return {59, 61, CASTLING};

    if (promo > 0) {
        static const int POLY_TO_FLAG[5] = {NONE, PROMOTEKNIGHT, PROMOTEBISHOP, PROMOTEROOK, PROMOTEQUEEN};
        return {engFrom, engTo, POLY_TO_FLAG[promo]};
    }

    return {engFrom, engTo, NONE};
}

std::optional<std::tuple<int, int, int>> OpeningBook::probe(const Board& board) {
    if (!loaded || entries.empty()) return std::nullopt;

    uint64_t hash = computePolyglotHash(board);
    ...
    if (it == entries.end() || it->key != hash) return std::nullopt;

    // Collect all matching entries and pick one weighted by weight
    uint32_t total = 0;
    auto start = it;
    ...
    for (auto cur = start; cur != it; ++cur) {
        cumulative += cur->weight;
        if (pick < cumulative)
            return decodeMove(cur->move);
    }

    return decodeMove(start->move); // fallback
}
